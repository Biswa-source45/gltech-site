...
export default function Footer({ onNavigate }) {
  ...
  const services = [
    ...
    { name: 'Digital Enterprise Application', id: 'digital-enterprise' }
  ];

  return (
    <footer className="bg-[#091E42] text-slate-300 font-sans border-t border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center cursor-pointer" onClick={() => handleLinkClick('home')}>
              <div className="bg-white p-1.5 rounded-lg w-[180px] flex items-center justify-center">
                <img src={logo} alt="GLOBX Logo" className="h-10 w-auto object-contain" />
              </div>
            </div>
            <div className="flex items-center justify-between w-[180px] pt-1">
              <a href="https://www.linkedin.com/company/globx-system-and-network-pvt-ltd" target="_blank" rel="noopener noreferrer" className="w-[52px] h-[38px] flex items-center justify-center bg-slate-800 hover:bg-blue-700 text-white rounded-lg transition-all" aria-label="LinkedIn">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.779-1.75-1.75s.784-1.75 1.75-1.75 1.75.779 1.75 1.75-.784 1.75-1.75 1.75zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clipRule="evenodd" />
                </svg>
              </a>
              <a href="http://x.com/GlobxSystem" target="_blank" rel="noopener noreferrer" className="w-[52px] h-[38px] flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-white rounded-lg transition-all" aria-label="Twitter">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a href="https://instagram.com/globxsystem" target="_blank" rel="noopener noreferrer" className="w-[52px] h-[38px] flex items-center justify-center bg-slate-800 hover:bg-pink-600 text-white rounded-lg transition-all" aria-label="Instagram">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
          ...
          {/* Column 3: Services */}
          <div>
            <h3 className="text-white font-display font-bold text-base mb-6 tracking-wide uppercase text-sm border-l-4 border-blue-500 pl-3">Services</h3>
            <ul className="space-y-3 text-sm">
              {services.map((item) => (
                <li key={item.id}>
                  <button 
          ...
          {/* Column 4: Contact Us */}
          <div className="space-y-6">
            <h3 className="text-white font-display font-bold text-base tracking-wide uppercase text-sm border-l-4 border-blue-500 pl-3">Get In Touch</h3>
            <ul className="space-y-4 text-sm text-slate-400">
              ...
              <li className="flex gap-3 items-center">
                ...
                <a href="tel:+919124668102" className="hover:text-white transition-colors">+91 0674 2546464</a>
              </li>
              <li className="flex gap-3 items-center">
                <Mail className="w-5 h-5 text-blue-500 shrink-0" />
                <a href="mailto:business@globx.co.in" className="hover:text-white transition-colors">business@globx.co.in</a>
              </li>
      ...
      <div className="bg-[#04142D] py-6 border-t border-slate-800/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-slate-500">
          <div>
            &copy; {new Date().getFullYear()} GLOBX System & Network Pvt. Ltd. All Rights Reserved.
          </div>
          <div className="flex gap-6">
            <button onClick={() => handleLinkClick('home')} className="hover:text-slate-300 cursor-pointer">Home</button>
            <button onClick={() => handleLinkClick('about')} className="hover:text-slate-300 cursor-pointer">About Us</button>
            <button onClick={() => handleLinkClick('contact')} className="hover:text-slate-300 cursor-pointer">Contact Us</button>
            <button onClick={() => handleLinkClick('careers')} className="hover:text-slate-300 cursor-pointer">Careers</button>
          </div>
        </div>
      </div>
    </footer>
